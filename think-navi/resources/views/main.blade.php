...
@section('content')
@include('components.header.main_header')
<div class="white-area flex justify-center">
    <div class="map container flex h-screen w-screen overflow-hidden">
                    ...
                    <div id="{{ $select_think->id }}thinkDropdown" class="hidden z-50 w-auto bg-gray-800 divide-y divide-gray-100 shadow">
                        <ul class="text-sm text-gray-700 flex">
                            ...
                            <li class="flex items-center justify-between">
                                <button data-modal-toggle="{{ $select_think->id }}modalMemo" type="button" class="btn-open-modal flex items-center justify-center bg-gray-800 hover:bg-gray-600 focus:outline-none" style="height: 40px; width: 40px;">
                            ...
                            <li class="flex items-center justify-between">
                                <button data-modal-toggle="{{ $select_think->id }}naviModal" type="button" class="btn-open-modal flex items-center justify-center bg-gray-800 hover:bg-gray-600 focus:outline-none" style="height: 40px; width: 40px;">
            ...
            <div class="next-child-area flex items-start flex-col">
                ...
                <div class="nav-group-area flex items-center relative">
                    ...
                    <div class="my-2">
                        ...
                        <div class="flex flex-col categroy-group-area rounded border-solid border-4 @if($categoryGroup->category_group === '6つの視点から考える') border-blue-700 @elseif($categoryGroup->category_group === '8つに派生させる') border-purple-700 @elseif($categoryGroup->category_group === '希望、欠点から考える') border-green-700 @elseif($categoryGroup->category_group === '類似から考える') border-orange-700 @elseif($categoryGroup->category_group === '原因、背景から考える') border-pink-700 @elseif($categoryGroup->category_group === '5W1Hから考える') border-yellow-700 @endif">
        ...
        @else
        <div id="map-area" class="content-area flex items-center absolute top-0 left-0">
            <form id="parent" method="post" class="flex group items-center m-0" action="{{ route('main.new-parent') }}">
                @csrf
                <input class="edit-file-id" type="hidden" name="edit_file_id" value="{{ optional($select_think_file)->id }}">

                <div class="relative inline-block my-2">
                    <div class="invisible inline-block px-4 py-3"></div>
                    <input name="edit_content" type="text" class="edit-content new-parent selected-content h-auto parent text-center outline-none rounded border-dashed border-4 border-black absolute top-0 left-0 w-full px-3 py-2 z-10" autofocus>
                </div>
            </form>
        </div>
        @endif
